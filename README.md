...
* **Bcrypt** - Biblioteca de criptografia de terceiros utilizada para gerar hashes seguros (`BLOB`) das senhas dos usuários.

---

## 🧠 Aprendizados e Tomadas de Decisão

Como este projeto reflete uma etapa importante da minha evolução na programação, algumas decisões técnicas foram tomadas de forma estratégica:
...
2. **Tratamento Rigoroso de Exceções:** Todas as funções que interagem com o banco de dados utilizam blocos `try/except/finally` para garantir que conexões e cursores nunca fiquem travados na memória da máquina caso ocorra uma falha crítica.
