...
def criar_tabela():
    ...
    try:
        ...
        # garante a liberação de recurso independente do resultado.
        if conexao:
...
def cadastrar_usuario(nome, user_name, data_nasc, email, senha):
    ...
    try:
        ...
        if verificar_registro("nome_de_usuario", user_name):
        ...
        if verificar_registro("email", email):
        ...
        # Padroniza os nomes para que apenas a primeira letra seja maiúscula
        nome = nome.title()
...
def verificar_registro(coluna, busca): 
    """
    Verifica a existência do valor informado em uma coluna específica da tabela 'usuario'.

    Argumentos: 
        coluna (str): Nome da coluna onde será feita a busca.
        busca (str): Valor a ser verificado na coluna.

    Retorna: 
        bool: True se o valor procurado existir, False caso contrário. 
    """  
    try:
        cmd = f'''SELECT 1 FROM usuario WHERE {coluna} = ?'''
        #Abre a conexão com o banco de dados e executa a busca do registro.
        conexao, cursor = iniciar_conexao()
        cursor.execute(cmd, (busca,))
        ...
        print(f"Ocorreu um erro ao verificar registro no banco de dados: {erro}")
        return False
...
def executar_busca_referencial(coluna_busca, coluna_filtro, valor_ref):
    """
    Executa a busca de uma informação específica com base em outra coluna de referência.

    Argumentos: 
        coluna_busca (str): Nome da coluna cujo valor será retornado.
        coluna_filtro (str): Nome da coluna usada como critério de busca.
        valor_ref (str): O valor que será buscado na coluna de filtro.

    Retorna: 
         string ou None: Retorna o dado encontrado como string ou None se não houver registro ou falhar.
    """
    try:
        # abre conexão com o banco e executa a busca passando os argumentos da referência.
        conexao, cursor = iniciar_conexao()
        cmd = f"SELECT {coluna_busca} FROM usuario WHERE {coluna_filtro} = ?"
        cursor.execute(cmd, (valor_ref,))
        retorno = cursor.fetchone()

        if retorno:
            # desempacota e retorna o valor encontrado. 
            valor = retorno[0]
            return valor
        ...
        print(f"Ocorreu um erro ao executar busca: {erro}")
        return None
